...
export async function fetchGitHubContent(
  ...
  } // GitHub Personal Access Token を環境変数から取得 (.env または .env.local から読み込まれる)
  const githubPat = import.meta.env.VITE_GITHUB_PAT;

  const headers: HeadersInit = {
    Accept: "application/vnd.github.v3+json", // APIバージョン指定
  }; // 環境変数に PAT が設定されていれば Authorization ヘッダーを追加
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error(errorMessage); // 404 エラーで、かつ特定の ref を指定していた場合（ref が undefined でも空文字でもない場合） // 無限ループを防ぐため、デフォルトブランチへの取得は一度だけ試みる
      if (response.status === 404 && ref !== undefined && ref !== "") {
        ...
        try {
          ...
          const fallbackContent = await fetchGitHubContent(
            ...
            path,
            undefined
          ); // ref を undefined にして再帰呼び出し // console.log(`Successfully fetched content for path "${path}" from default branch after 404 fallback.`);
          ...
          console.error(
            ...
            fallbackError
          ); // フォールバック失敗ログは残す // 最終的に元の 404 エラーを含む新しいエラーをスロー
          throw new Error(
